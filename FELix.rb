...
def debug_packet(packet, dir)
	if packet[0..3] == "AWUC" and packet.length == 32
		...
		puts "<-- AWFELVerifyDeviceResponse: %s, FW: %d, mode: %s" % [board_id_to_str(p.board), p.fw, FEL_DEVICE_MODE.key(p.mode)]
	else
		return if dir == :unk
		...
		if packet.length == 16
			...
			case p.cmd
			when AWCOMMAND[:FEL_R_VERIFY_DEVICE]
				puts "FEL_R_VERIFY_DEVICE"
			when AWCOMMAND[:FEX_CMD_FES_RW_TRANSMITE]
				puts "#{AWCOMMAND.key(p.cmd)}: " << (p.data_tag and FESTransmiteFlag::FES_W_DOU_DOWNLOAD ? "FES_TRANSMITE_W_DOU_DOWNLOAD" : (p.data_tag and FESTransmiteFlag::FES_R_DOU_UPLOAD ? "FES_TRANSMITE_R_DOU_UPLOAD" : "FES_TRANSMITE_UNKNOWN #{p.flag}"))
			else
				puts "#{AWCOMMAND.key(p.cmd)}: (0x%.2X): #{packet.to_hex_string[0..46]}" % p.cmd
			end
		else
			print "\n"
...
def debug_packets(file)
	...
	contents.scan(/^.*?{(.*?)};/m) { |packet|
        hstr = ""
        packet[0].scan(/0x(\w{2})/m) { |hex|
            hstr << hex[0]
        }
        #Strip USB header
...
def aw_list_devices(context)
	i = 1
	devices.each { |d|
		puts "* %2.d: (port %d) FEL device %d@%d %x:%x" % [i, d.port_number, d.bus_number, d.device_address, d.idVendor, d.idProduct ]
		++i
	}
end
...
begin
		puts "FEL".red << "ix" << " version 1.0 ALFA"
	...
	if devices.size > 1 and $options[:device] == nil # If there's more than one device list and ask to select
...
puts "Opening the device"
handle = devices[$options[:device]].open
...
r = handle.bulk_transfer(:dataOut => request.to_binary_s,
:endpoint => (devices[$options[:device]].endpoints[0].direction == :out ? devices[$options[:device]].endpoints[0] : devices[$options[:device]].endpoints[1])
)
puts "Result: " << r.inspect

data = AWFELStandardRequest.new
puts "Sending %s" % data.to_binary_s.to_hex_string
r = handle.bulk_transfer(:dataOut => data.to_binary_s,
:endpoint => (devices[$options[:device]].endpoints[0].direction == :out ? devices[$options[:device]].endpoints[0] : devices[$options[:device]].endpoints[1])
)
puts "Result: " << r.inspect

r = handle.bulk_transfer(:dataIn => 13,
:endpoint => (devices[$options[:device]].endpoints[0].direction == :in ? devices[$options[:device]].endpoints[0] : devices[$options[:device]].endpoints[1])
)
...
r = handle.bulk_transfer(:dataIn => 32,
:endpoint => (devices[$options[:device]].endpoints[0].direction == :in ? devices[$options[:device]].endpoints[0] : devices[$options[:device]].endpoints[1])
)
